    ...
    <div class="container-fluid">
        <div class="d-flex justify-content-between align-items-center">
            <ul class="nav nav-tabs page-header-tab">
                <li class="nav-item"><a class="nav-link active" href="javascript:void(0)">Tenant Status List</a></li>
            </ul>
            <div class="header-action">
                <a class="btn btn-primary" href="{{ route('tenant-status.create') }}"><i class="fe fe-plus mr-2"></i> Add</a>
            </div>
        </div>
    </div>
</div>
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover js-basic-example dataTable table_custom border-style spacing5">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td>{{ $statlist->id }}</td>
                                        <td>{{ $statlist->status_name }}</td>
                                        <td class="actions">
                                            <a href="{{ route('tenant-status.show', $statlist->id) }}"><button class="btn btn-sm btn-icon on-default m-r-5" data-toggle="tooltip" data-original-title="Show"><i class="icon-doc" aria-hidden="true"></i></button></a>
                                            ...
                                            <form method="post" action="{{ route('tenant-status.destroy', $statlist->id) }}">
                                                @method('DELETE')
