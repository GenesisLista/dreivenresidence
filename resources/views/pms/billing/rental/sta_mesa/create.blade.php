    ...
    <div class="container-fluid">
        <div class="d-flex justify-content-between align-items-center">
            <ul class="nav nav-tabs page-header-tab">
                ...
                <li class="nav-item"><a class="nav-link active" href="{{ route('rental-sta-mesa.index') }}">Sta. Mesa</a></li>
...
<div class="section-body mt-3">
    <div class="container-fluid">
        <form action="{{ route('rental-sta-mesa.store') }}" method="POST" enctype="multipart/form-data" autocomplete="false">
            {{ csrf_field() }}
            <div class="card">
                <div class="card-body">
                    <div class="row clearfix">
                        ...
                        <div class="col-12">
                            ...
                            <a class="btn btn-warning" href="{{ route('rental-sta-mesa.index') }}"><i class="fe fe-arrow-left mr-2"></i>Back</a>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-hover js-basic-example dataTable table_custom border-style spacing5">
                                ...
                                <tbody>
                                    @foreach($bill_rental_sta_mesa as $brstamesa)
                                    <tr>
                                        <td>{{ $brstamesa->apartment->room }}</td>
                                        <td>{{ $brstamesa->tenant->name }}</td>
                                        <td>{{ number_format($brstamesa->billed_amount) }}</td>
                                        <td>{{ $brstamesa->billed_date_paid }}</td>
                                        <td>{{ number_format($brstamesa->billed_paid_amount) }}</td>
                                        <td>{{ number_format($brstamesa->billed_balance_amount) }}</td>
                                        <td>{{ $brstamesa->payment_status->name }}</td>
                                        <td class="actions">
                                            ...
                                            <form method="post" action="{{ route('rental-sta-mesa.destroy', $brstamesa->id) }}">
                                                @method('DELETE')
...
<script>
    jQuery(document).ready(function() {
        ...
        $('#apartment_id').on('change', function () {
            ...
            $.ajax({
                url: "{{ route('rental-sta-mesa.rental-list') }}",
