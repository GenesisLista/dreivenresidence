    ...
    <div class="container-fluid">
        <div class="d-flex justify-content-between align-items-center">
            <ul class="nav nav-tabs page-header-tab">
                ...
                <li class="nav-item"><a class="nav-link active" href="{{ route('rental-alr-building.index') }}">ALR Building</a></li>
            </ul>
        </div>
    </div>
</div>
...
<div class="section-body mt-3">
    <div class="container-fluid">
        <form action="{{ route('rental-alr-building.store') }}" method="POST" enctype="multipart/form-data" autocomplete="false">
            {{ csrf_field() }}
            <div class="card">
                <div class="card-body">
                    <div class="row clearfix">
                        ...
                        <div class="col-12">
                            ...
                            <a class="btn btn-warning" href="{{ route('rental-alr-building.index') }}"><i class="fe fe-arrow-left mr-2"></i>Back</a>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-hover js-basic-example dataTable table_custom border-style spacing5">
                                ...
                                <tbody>
                                    @foreach($bill_rental_alr_building as $brAlrBuilding)
                                    <tr>
                                        <td>{{ $brAlrBuilding->apartment->room }}</td>
                                        <td>{{ $brAlrBuilding->tenant->name }}</td>
                                        <td>{{ number_format($brAlrBuilding->billed_amount) }}</td>
                                        <td>{{ $brAlrBuilding->billed_date_paid }}</td>
                                        <td>{{ number_format($brAlrBuilding->billed_paid_amount) }}</td>
                                        <td>{{ number_format($brAlrBuilding->billed_balance_amount) }}</td>
                                        <td>{{ $brAlrBuilding->payment_status->name }}</td>
                                        <td class="actions">
                                            ...
                                            <form method="post" action="{{ route('rental-alr-building.destroy', $brAlrBuilding->id) }}">
                                                @method('DELETE')
...
<script>
    jQuery(document).ready(function() {
        ...
        $('#apartment_id').on('change', function () {
            ...
            $.ajax({
                url: "{{ route('rental-roxas-district.rental-list') }}",
