...
<div class="section-body mt-3">
    <div class="container-fluid">
        <form action="{{ route('rental-roxas-district.store') }}" method="POST" enctype="multipart/form-data" autocomplete="false">
            {{ csrf_field() }}
            <div class="card">
                <div class="card-body">
                    <div class="row clearfix">
                        ...
                        <div class="col-12">
                            ...
                            <a class="btn btn-warning" href="{{ route('rental-roxas-district.index') }}"><i class="fe fe-arrow-left mr-2"></i>Back</a>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-hover js-basic-example dataTable table_custom border-style spacing5">
                                ...
                                <tbody>
                                    @foreach($bill_rental_roxas_district as $brRoxasDistrict)
                                    <tr>
                                        <td>{{ $brRoxasDistrict->apartment->room }}</td>
                                        <td>{{ $brRoxasDistrict->tenant->name }}</td>
                                        <td>{{ number_format($brRoxasDistrict->billed_amount) }}</td>
                                        <td>{{ $brRoxasDistrict->billed_date_paid }}</td>
                                        <td>{{ number_format($brRoxasDistrict->billed_paid_amount) }}</td>
                                        <td>{{ number_format($brRoxasDistrict->billed_balance_amount) }}</td>
                                        <td>{{ $brRoxasDistrict->payment_status->name }}</td>
                                        <td class="actions">
                                            ...
                                            <form method="post" action="{{ route('rental-roxas-district.destroy', $brRoxasDistrict->id) }}">
                                                @method('DELETE')
